                ...
                <div className="p-6 pb-20">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        {packages.map((pkg, index) => (
                            <div 
                                key={index} 
                                className={`relative bg-slate-900/40 backdrop-blur-sm border border-red-500/20 rounded-xl p-6 hover:bg-slate-900/60 transition-all duration-300 hover:scale-105 hover:border-red-400/40 hover:shadow-lg hover:shadow-red-500/10 ${
                                    pkg.popular ? 'ring-2 ring-red-400/30' : ''
                                }`}
                            >
                                {/* Popular Badge */}
                                {pkg.popular && (
                                    <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
                                        <span className="bg-gradient-to-r from-red-400 to-red-600 text-white text-xs font-bold px-4 py-1 rounded-full shadow-lg">
                                            MOST POPULAR
                                        </span>
                                    </div>
                                )}
                                ...
                                <div className="text-center mb-6">
                                    ...
                                    <div className="text-slate-300">
                                        <p className="text-sm bg-slate-800/50 rounded-lg px-3 py-1 inline-block border border-red-500/20">
                                            {pkg.minInvestment} - {pkg.maxInvestment}
                                        </p>
                                        <div className="flex items-center justify-center mt-3">
                                            <p className="text-2xl font-bold text-red-400">{pkg.roi}</p>
                                            <span className="text-red-400 ml-2 flex items-center bg-red-500/10 px-2 py-1 rounded-lg border border-red-500/20">
                                ...
                                <div className="space-y-3 mb-6">
                                    {pkg.features.map((feature, idx) => (
                                        <div key={idx} className="flex items-center space-x-3 bg-slate-800/30 rounded-lg p-3 border border-red-500/10 hover:border-red-500/20 transition-colors duration-200">
                                ...
                                {/* Invest Button */}
                                <button className="w-full bg-gradient-to-r from-red-400 to-red-600 hover:from-red-500 hover:to-red-700 text-white font-semibold py-3 rounded-lg transition-all duration-200 shadow-lg hover:shadow-red-500/20 hover:scale-105 border border-red-500/20">
