  ...
  return (
    <>
      <div className="w-full h-[83vh] px-5 py-8 phone:px-4">
          ...
          <div className="flex gap-5 text-sm font-semibold border-b border-[#2e2e2e]">
            {["profile", "security"].map((tab) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  isActive
                    ? "pb-2 border-b-2 border-[white] text-[white]"
                    : "text-[#bfbfbf] hover:text-[white] transition-all"
                }
              >
        ...
        <div className="mt-5 w-full h-[320px] phone:h-[400px] bg-[#0e0e0e]/80 border border-[white50] rounded-xl backdrop-blur-md overflow-auto p-4">
          <Outlet />
        </div>
      </div>
    </>
